...
declare -r DOTFILES_UTILS_URL="https://raw.githubusercontent.com/$GITHUB_REPOSITORY/master/os_x/utils.sh"
...
main() {
    ...
    if [ -x 'os_x/utils.sh' ]; then
        source 'os_x/utils.sh' || exit 1
    else
    ...
    if answer_is_yes; then
      ./os_x/create_directories.sh
    fi

    # --------------------------------------------------------------------------

    print_info 'Create symbolic links'
    ./os_x/create_symbolic_links.sh
    ...
    if cmd_exists 'git'; then
        ...
        if [ "$(git config --get remote.origin.url)" != "$DOTFILES_ORIGIN" ]; then
            ...
            ./os_x/initialize_git_repository.sh "$DOTFILES_ORIGIN"
        fi
